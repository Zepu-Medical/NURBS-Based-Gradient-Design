...
for c1=1:length(NURBS_curve)
    for c2=1:length(NURBS_curve(c1).Knot)
        ...
        ind_eq{1,1}=[ind_eq{1,1},l0+3*(1:n-1)]; % 连接点处一阶连续
        %
            if NURBS_curve(c1).ContrPoint{c2}(1,1)<1e-4
            ind_eq{2,1}=[ind_eq{2,1},l0+1];  %  phi=0处的边界导线
            end
        ...
        if any(NURBS_curve(c1).WireId(c2,1)==NURBS_curve(c1+1).WireId.')
            ...
            ind_eq{3,1}=[ind_eq{3,1},l0+1];  %  不同电流面上相等的导线点
            elseif NURBS_curve(c1).ContrPoint{c2}(end,2)>(1-1e-4)
            ind_eq{3,1}=[ind_eq{3,1},l0+2*n+2];  %  不同电流面上相等的导线点
            end
        end
        end
        ...
        if any(NURBS_curve(c1).WireId(c2,1)==NURBS_curve(c1-1).WireId.')
            ...
            ind_eq{4,1}=[ind_eq{4,1},l0+1];  %  不同电流面上相等的导线点
            elseif NURBS_curve(c1).ContrPoint{c2}(end,2)<4e-3
            ind_eq{4,1}=[ind_eq{4,1},l0+2*n+2];  %  不同电流面上相等的导线点
            end
