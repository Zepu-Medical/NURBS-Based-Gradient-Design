...
if ~exist('coeff_iter','var')||(exist('coeff_iter','var')&&isempty(coeff_iter))
    coeff_iter=cell(0,1);
    coeff_0=coeff_opt0;
else
    coeff_0=coeff_iter{end};
end
...
Gbias_r=0.054*ones(1,size(target_para,1));   %0.049
...
%%
    ...
    % checkGradients(@(x1)Contrain_field_and_continuous_v03(x1,WirePara,NonlinEq,OptPara,ConsPara),coeff_wire,opts,'IsConstraint',true,'Display','on');
    % 
...
Cell_wire_ini = nurbs_fold2all(NURBS_curve_ini,WirePara.size_coil,WirePara.fold_condition);
     ...
     PlotPara.DSVSize=[0.5,0.5,0.5];
     PlotPara.PlotRange={[-0.5,0.5],0,[-0.5,0.5];[-0.5,0.5],[-0.5,0.6],0.233;[-0.5,0.5],[-0.5,0.6],0.18;[-0.5,0.5],[-0.5,0.6],0.11};
...
if 1
    ...
    [min(distance_opt{1}),min(distance_opt{2})]
end
