...
id=0;
dis_para=cell(0,5);
d0_rang=[0.0041,0.004,0.008]*1.05;  % [0.0039,0.005,0.012,0.004]
ratio_rang=[1,0.01,0.5];    
ratio1_rang=[2,0.5,1,1.5]*50;
d0=[];
d_reg={};
ratio={};
ratio1=[];
...
for c0=1:length(NURBS_curve)
    ...
    for c1=1:size(tmp_dis,1) 
        ...
        tmp_ratio{c1,1}=ratio_rang(c0)*exp(-NURBS_curve(c0).dis{c1,6}/(d0_rang(c0)*2));  % 权重值
    end
