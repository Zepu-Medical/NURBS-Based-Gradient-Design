...
Gbias_r=0.056*ones(1,size(target_para,1));   
...
torque_range0=[-40,40;];
    torque_range=torque_range0+[10,-10]*4+[-0.01,0.01];
...
if  strcmpi(ConsPara.FieldVP,'on')
        ...
        [PPhi,ZZ]=ndgrid(rang.phi,linspace(l_z/2,0.0,ConsPara.field_size(2)));
        elseif strcmpi(WirePara.fold_condition,'4-fold')
        rang.z=linspace(0,1,ConsPara.field_size(2));
        [PPhi,ZZ]=ndgrid(rang.phi,linspace(l_z/2,-l_z/2,ConsPara.field_size(2)));
        end
        ...
        ConsPara.A_l=-0.8/8*repmat(g0*r_t*cos(phi_t).',[length(z_t),1]);     
...
%% 
[f,gf]=Objective_phase2(coeff_wire,OptPara,WirePara_d,1);
...
if 1
    ...
    NURBS_surf(1).KnotChange([5,9,10,14])=1;
    NURBS_surf(3).KnotChange([10])=1;
...
figure
    ...
    Coil2D_ini(3).seg_idx{1,1}=[1	15	30	46	64	79	94	109	121	145];
