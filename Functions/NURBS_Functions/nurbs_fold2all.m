function Cell_wire = nurbs_fold2all(NURBS_surf_fit,size_coil,FoldCondition,seg_n)
...
Cell_wire_temp=cell(0,1);
Cell_wire_surf_temp=cell(0,1);
...
for c1=1:length(NURBS_surf_fit)

    for c2=1:length(NURBS_surf_fit(c1).Knot)

    n=NURBS_surf_fit(c1).Knot{c2,1}(end);

    [nurbs_fold,~]=Func_NURBSCurve(linspace(0,n,1+seg_n*n),NURBS_surf_fit(c1).Knot{c2,1},[NURBS_surf_fit(c1).ContrPoint{c2,1},NURBS_surf_fit(c1).Weight{c2,1}],2,'Exponential');
   
   
    temp_wire=[size_coil(c1,1)*(1-nurbs_fold(:,2))+size_coil(c1,3)*nurbs_fold(:,2),...
        nurbs_fold.*[1,size_coil(c1,4)-size_coil(c1,2)]+[0,size_coil(c1,2)]];  %变换到方位角和z坐标。
    surf_size{end+1,1}=repmat(size_coil(c1,:),[size(temp_wire,1)-1,1]);% c1*ones(size(temp_wire,1)-1,1);
    surf_id{end+1,1}=c1*ones(size(temp_wire,1)-1,1);
    Cell_wire_fold{end+1,1}=temp_wire;
    Wire_Id(end+1,1)=NURBS_surf_fit(c1).WireId(c2);
            ...
            while(length(ind_tmp)>=l0)
                for c2=1:length(ind_tmp)
                    ...
                    if sqrt(v1(1)^2+v2(1)^2-2*v1(1)*v2(1)*cos(v1(2)-v2(2))+(v1(3)-v2(3))^2)<d0
                        ...
                        temp_surf_size=[surf_size{ind_tmp(c2),1};temp_surf_size];
...
for c1=1:size(Cell_wire_temp,1)
        ...
        nurbs_id=[nurbs_id;flip(nurbs_id(:,:),1)];
