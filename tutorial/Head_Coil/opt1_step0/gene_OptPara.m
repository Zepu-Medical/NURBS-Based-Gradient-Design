...
% limitations under the License.
    ...
    % near_wire(1).Wire_arrange{10,2}=-1;
    % near_wire(1).Wire_arrange{11,3}=[9,10];
    % 
    % near_wire(3).Wire_arrange{3,2}=[4,34];
    % near_wire(3).Wire_arrange{4,3}=[3,34];
    % near_wire(3).Wire_arrange{15,2}=-1;
    % near_wire(3).Wire_arrange{16,3}=[3,4];
    % 
    % near_wire(4).Wire_arrange{5,2}=-1;
    % near_wire(4).Wire_arrange{6,3}=1;
...
d0_rang=[0.003,0.003,0.0072,0.003]*1.2; %[0.002,0.0045,0.004,0.002];   % [0.0014,0.0018,0.0032,0.0014]*2.5
ratio_rang=[1,0.05,0.3,0.1];    %[1,0.05,0.5]
ratio1_rang=[2,1,2,1.5]*50;  

% d0_rang=[0.004,0.004,0.008,0.004];
% ratio_rang=[1,0.05,0.5,0.2];
% ratio1_rang=[2,1,2,1.5]*50;
...
for c0=1:length(NURBS_curve)
    ...
    for c1=1:size(tmp_dis,1) 
        ...
        % tmp_ratio(c1,1)=ratio_rang(c0).*5/length(NURBS_curve(c0).dis{c1,2});
        % tmp_ratio{c1,1}=ratio_rang(c0)*50/length(NURBS_curve(c0).dis{c1,2})*exp(-NURBS_curve(c0).dis{c1,6}/(d0_rang(c0)*2));  % 权重值
        weight=ones(size(NURBS_curve(c0).dis{c1,6}));

        tmp_ratio{c1,1}=ratio_rang(c0)*weight.*exp(-NURBS_curve(c0).dis{c1,6}/(d0_rang(c0)*2));  % 权重值
 %       tmp_ratio{c1,1}(NURBS_curve(c0).dis{c1,6}>d0_rang(c0)*20)=0;
    end
...
for c1=1:size(dis_para,1)
    ...
    Xi2Cpn0=s0-flip(0:p)+1; %参数到控制点的转换矩阵
...
OptPara.N1=N1;
OptPara.dN1=dN1;
elseif strcmpi(Method,'Square Sum of Deviation')
