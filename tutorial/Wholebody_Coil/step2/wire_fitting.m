...
alpha_r={0.01*ones(length(Coil2D_ini(1).Wire2D),1),0.01*ones(length(Coil2D_ini(2).Wire2D),1)};
alpha_r{1}(2)=0.1;
f_eval=[];
...
%     hold on
    ...
    parfor c1=1:length(Wire2D_manu)
...
WirePara.size_coil(:,2)=abs(WirePara.size_coil(:,2));
WirePara.size_coil(:,4)=0;
