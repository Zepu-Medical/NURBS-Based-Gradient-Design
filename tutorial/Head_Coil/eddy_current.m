...
%    Temp_wireposition(:,2)=Temp_wireposition(:,2)-pi/2;
...
parfor c2=1:length(r_n)
    ...
    for c1=1:length(Num_seg)-1
        ...
        A_phi=(-dl(:,1).*sin(phi_fp(:)).'+dl(:,2).*cos(phi_fp(:)).').*Dis_si;    %源和phi方向电流的互感
        A_phi=reshape(RRho*l_z(1)*sum(A_phi,1),size(Z_fp));

        A_z=dl(:,3).*Dis_si;    %源和z方向电流的互感
...
r_t=0.15;
% r_screen=0.453;
z_t=[0.13,0.065,0,-0.065,-0.13];
phi_t=(0:7)/4*pi;%[0,pi/4,pi/2];
...
m_range={-5:5,-7:-1,0,1:2:11};

for m=m_range{ind}
    Driving=cell2mat(Driving_c);
    Driving_m=Driving(:,rang.m==m);
    % [NN,MM]=ndgrid(1:Order_z,Order_phi+1+m);
    % ind=sub2ind([Order_z,2*Order_phi+1],NN(:),MM(:));
    % Driving_m=cell(length(r_n),1);
    % for c1=1:length(r_n)
    %     Driving_m{c1}=Driving{c1}(ind);
    % end
    % Driving_m=cell2mat(Driving_m);
    ...
    U0=Mat_Mutual_inv.*cell2mat(Resistance(:,1)).';   % U0=Mat_Mutual_inv*diag(cell2mat(Resistance(:,1)));
%    max(U1-U0,[],"all")
    % [E_vector,E_value]=eig((U0+U0.')/2);
...
G=109.8e-6;
display(num2str(100*max(abs(Bz_time(:,2)/G/r_t)),3))
% figure
% plot(0.005*(0:125),100*Bz_time(1,1:126)/(G*r_t))
% hold on
% %plot(0.002*(0:125),100*Bz(5,1:126)/(G*r_t))
...
% plot(0.005*(0:125),100*Bz_time(26,1:126)/(G*r_t))
% plot(0.005*(0:125),100*Bz_time(34,1:126)/(G*r_t))
% % ylim([-1,1])
% xlabel('t/s')
% ylabel('x relative field/%')
