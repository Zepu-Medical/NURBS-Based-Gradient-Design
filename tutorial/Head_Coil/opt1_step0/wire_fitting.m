...
% sz=[size_c(1,1),abs(size_c(1,2)-size_c(1,4));(size_c(2,1)+size_c(2,3))/2,size_c(2,2)-size_c(2,4);size_c(3,1),abs(size_c(3,2)-size_c(3,4))];  %半径和z方向尺寸
sz=[(size_c(:,1)+size_c(:,3))/2,abs(size_c(:,2)-size_c(:,4))];  %半径和z方向尺寸
for c1=1:length(Coil2D_ini)
alpha_r{c1,1}=0.01*ones(length(Coil2D_ini(1).Wire2D),1);
end
alpha_r{2}(end)=0.01;
f_eval=[];
...
%         p1_lb=-0.01*ones(Ncp,1);
%         p1_rb=pi/2*sz(c0,1)*ones(Ncp,1);
%         p2_lb=-0.01*ones(Ncp,1);
%         p2_rb=1.0*sz(c0,2)*ones(Ncp,1);
         w_lb=-0.2*ones(Ncp,1);
         w_rb=0.3*ones(Ncp,1);
        w_lb(1)=0;
        w_rb(1)=0;
...
%        lb=[lb;-inf*ones(3*Ncp,1);];
%       rb=[rb;inf*ones(3*Ncp,1);];
        condition=1;
        ...
        xi2cp=xi_span-flip(0:2)+1; %参数到控制点的转换矩阵
...
% n1=37;
% n2=2;
% coeff1(n1)=coeff1(n1)+d;
% [nceq,ceq,gnceq,gceq]=Constrain_NURBSfitting(coeff_ini,ind_eq,Npara)
% [nceq,ceq1,gnceq,gceq]=Constrain_NURBSfitting(coeff1,ind_eq,Npara);
% (ceq1(n2)-ceq(n2))/d-gceq(n1,n2)


% Plot_Wire2D(Coil2D_ini);
% NURBS_plot(NURBS_curve_fit0,{'-'},'control point');

%% 处理拟合后的NURBS数据 
% 1 手动修改，主要是修改部分导线的奇异弯折。


% NURBS_plot(NURBS_curve_modified,{'-'},'control point');

% 穿过0.26左右，正负0.04比较安全。
% Wire2D_ini=cell(0,1);
% 
% range=[1,37,74,110]  % 1/8曲线的范围
% for c0=1:Coil2D
% for c1=1:length(Wire2D_manu)
%     Wire_temp=Wire2D_manu{c1};
%     Wire_temp(Wire_temp(:,1)<pi/2,:)=[];
%     Wire_temp(1,1)=pi/2;
%     Wire_temp(end,1)=pi/2;
%     for c2=0:floor(length(range)/2)-1
%         if range(2*c2+1)<Wire_temp(1,2) & Wire_temp(1,2)<range(2*c2+2)
%             Wire_temp1=[];
%             Wire_temp1(:,1)=(Wire_temp(:,1)*2-pi)/pi;
%             Wire_temp1(:,2)=(Wire_temp(:,2)-range(2*c2+1))/(range(2*c2+2)-range(2*c2+1));
%             Wire2D_ini{end+1,1}=Wire_temp1;
% 
%             continue
%         end
%     end
...
WirePara.size_coil=size_c;  %电流面尺寸
WirePara.fold_condition='4-fold';
WirePara.DSVr=[0.26;0.3];
WirePara.Shieldr=[0.345,0.345,0.6,-0.6];
...
ind_sort=[2,4,6,7,8,9,1,3,5];
NURBS_curve_modified(4).Xi=NURBS_curve_modified(4).Xi(ind_sort);
NURBS_curve_modified(4).Knot=NURBS_curve_modified(4).Knot(ind_sort);
NURBS_curve_modified(4).ContrPoint=NURBS_curve_modified(4).ContrPoint(ind_sort);
NURBS_curve_modified(4).Weight=NURBS_curve_modified(4).Weight(ind_sort);
NURBS_curve_modified(4).WireId=NURBS_curve_modified(4).WireId(ind_sort);
