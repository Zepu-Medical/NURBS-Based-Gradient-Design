        ...
        if c1==1||c1==3  %主线圈和屏蔽线圈
            ...
            temp_rhx(end)=phi_v(NURBS_curve(c1).WireId(c2,1));
                if any(NURBS_curve(c1).WireId(c2,1)==7)
                    % temp_lhx(2)=phi_v(NURBS_curve(c1).WireId(c2,1));
                    temp_rhx(end-1)=phi_v(NURBS_curve(c1).WireId(c2,1))+0.06;
                end
            ...
            temp_rhx(1)=phi_v(NURBS_curve(c1).WireId(c2,1));
                if any(NURBS_curve(c1).WireId(c2,1)==[3:2:9])
                    % temp_lhx(2)=phi_v(NURBS_curve(c1).WireId(c2,1));
