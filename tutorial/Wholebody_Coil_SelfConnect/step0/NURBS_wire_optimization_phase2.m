...
Gbias_r=0.096*ones(1,size(target_para,1));   %0.049
...
if  strcmpi(ConsPara.FieldVP,'on')
        ...
        ConsPara.A_l=-0.0035/4*repmat(g0*r_t*cos(phi_t*0).',[length(z_t),1]);
...
Cell_wire_opt= nurbs_fold2all(NURBS_curve_opt,WirePara.size_coil,WirePara.fold_condition,5);
...
if 0
figure
    for c1=1:length(NURBS_curve_opt)
        subplot(1,length(NURBS_curve_opt),c1)
        Coil2Dfit(c1)=NURBS_plot(NURBS_curve_opt(c1),{'-'},'control point');
        xlim([0,pi/2]);
    end
end
...
if 1
    ...
    NURBS_curve_opt = NURBS_formal(NURBS_curve_opt,FormPara);
    NURBS_curve_opt=find_mindistance(NURBS_curve_opt,WirePara,near_wire,5,'off');
...
if 1
    ...
    NURBS_surf=NURBS_curve_opt;
     ...
     NURBS_surf(1).KnotChange([17:19])=1;
...
figure
   ...
   Cell_wire_opt=wire_fold2all(Coil2D_ini,WirePara.size_coil,WirePara.fold_condition);
    Cell_wire_eddy=wire_fold2all(Coil2D_ini,WirePara.size_coil,WirePara.fold_condition);
end
...
if 1
    ...
    FastHenry_import(Cell_wire_opt,'standard_60cm_opt',WirePara);
